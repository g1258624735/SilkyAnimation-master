...
public class FasterAnimationsContainer {
    ...
    public static FasterAnimationsContainer getInstance(ImageView imageView) {
        if (sInstance == null) {
            ...
            sInstance.mSoftReferenceImageView = null;
            sInstance.mSoftReferenceImageView = new SoftReference<>(imageView);
        }
    ...
    private AnimationFrame getNext() {
        mIndex++;
        if (mIndex >= mAnimationFrames.size()) mIndex = 0;
...
//            }
            ...
            mRecycleBitmap = BitmapFactory.decodeResource(mImageView.getContext().getResources(), params[0], options);
